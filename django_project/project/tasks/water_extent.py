...
def compute_water_extent_task(
    ...
    try:
        ...
        result = calculate_water_extent_from_tif(
            awei_output.file.path, threshold=threshold
        )
        ...
        task.add_log(
            f"Water surface area calculated: {result['area_km2']} km²"
        )
        task.complete()
...
def generate_water_mask_task(
    ...
    try:
        ...
        result = generate_water_mask_from_tif(
            awei_output.file.path, threshold=threshold
        )
